...
def test_statistics_no_duplicate_project_class():
    ...
    assert dupes == 0, \
        f"Found {dupes} duplicate clip+annotation+class rows in Statistics"
...
# ---------------------------------------------------------------------------
# Regression tests — re-export with all_points and compare to committed file
# ---------------------------------------------------------------------------

def test_export_produces_valid_output(tmp_path):
    out = str(tmp_path / "export_all_points.xlsx")
    export_to_excel(
        ml_data_folder=str(HDF5_DIR),
        output_excel=out,
        contour_method="all_points",
    )
    assert Path(out).exists(), "No output file produced"
    df = pd.read_excel(out, sheet_name="Contours")
    assert len(df) > 0, "Contours sheet is empty"
    assert df.notna().any().any(), "Contours sheet is all-null"

def test_export_regression_row_counts(tmp_path):
    """Re-exporting with all_points should produce the same row counts."""
